...
# --- [削除] ここにあった InvalidInputError のクラス定義は削除されました ---
...
def _validate_travel_costs(travel_costs_matrix: Dict[Tuple[str, str], int], valid_classroom_ids: Set[str]) -> None:
    ...
    for key, value in travel_costs_matrix.items():
        ...
        if not isinstance(key, tuple) or len(key) != 2: raise InvalidInputError(f"{context}: キーは2つの要素を持つタプルである必要があります。")
        from_id, to_id = key
        if not isinstance(from_id, str) or not isinstance(to_id, str): raise InvalidInputError(f"{context}: キーの要素は両方とも文字列である必要があります。")
        if from_id not in valid_classroom_ids: raise InvalidInputError(f"{context}: 出発教室ID '{from_id}' が既存の教室IDに存在しません。")
        if to_id not in valid_classroom_ids: raise InvalidInputError(f"{context}: 到着教室ID '{to_id}' が既存の教室IDに存在しません。")
        if not isinstance(value, (int, float)) or value < 0: raise InvalidInputError(f"{context}: 値は0以上の数値である必要がありますが、'{value}' を受け取りました。")

def adapt_data_for_engine(lecturers_data: List[Dict[str, Any]], courses_data: List[Dict[str, Any]], classrooms_data: List[Dict[str, Any]], travel_costs_matrix: Dict[Tuple[str, str], int], **kwargs) -> Dict[str, Any]:
    logger = logging.getLogger(__name__)
    try:
        ...
        return {"lecturers_data": lecturers_data, "courses_data": courses_data, "classrooms_data": classrooms_data, "travel_costs_matrix": travel_costs_matrix}
...
def run_optimization_with_monitoring(lecturers_data: List[Dict[str, Any]], courses_data: List[Dict[str, Any]], classrooms_data: List[Dict[str, Any]], travel_costs_matrix: Dict[Tuple[str, str], int], **kwargs: Any) -> OptimizationResult:
    logger = logging.getLogger(__name__)
    solver_args = {"lecturers_data": lecturers_data, "courses_data": courses_data, "classrooms_data": classrooms_data, "travel_costs_matrix": travel_costs_matrix, **kwargs}
    for attempt in range(RETRY_LIMIT):
        ...
        try:
            ...
            if parent_conn.poll(PROCESS_TIMEOUT_SECONDS):
                ...
                if isinstance(result, Exception): raise InvalidInputError(f"最適化プロセスでエラーが発生しました: {result}") from result
        ...
        if solver_output is not None:
            ...
            for assignment in solver_output['assignments']:
                ...
                processed_assignments.append({
                    **assignment,
                    "講師名": lecturer.get("name", "不明"), "講座名": course.get("name", "不明"),
                    "教室ID": course.get("classroom_id", "不明"), "スケジュール": course.get("schedule", "不明"),
                    ...
                    "講座タイプ": course.get("course_type"), "講座ランク": course.get("rank"),
                })
        ...
        if attempt < RETRY_LIMIT - 1:
            logger.info("再試行します...")
            continue
    raise InvalidInputError("最適化処理が複数回の試行でも設定時間内に完了しませんでした。問題が解決しない場合は、入力データを簡素化するか、モデル設定を見直してください。")